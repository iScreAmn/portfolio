...
const { common, dashboard } = adminData;
...
const AnalyticsDashboard = ({ period }) => {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message || dashboard.loadFailed);
    } finally {
      setLoading(false);
    }
  }, [period]);
  ...
  if (loading) {
    return (
      <div className="analytics-container">
        <div className="analytics-loading">{dashboard.loading}</div>
      </div>
    );
  }
  ...
  return (
    <div className="analytics-container">
      <div className="analytics-header">
        <h2 className="analytics-title">{dashboard.title}</h2>
      </div>

      {!hasAnyData && <p className="analytics-empty">{dashboard.emptyPeriod}</p>}
      ...
      <div className="analytics-grid">
        ...
        <div className="analytics-card analytics-card--full">
          <div className="analytics-card__header">
            <h3 className="analytics-card__title">{dashboard.cards.dailyActivity}</h3>
          </div>
